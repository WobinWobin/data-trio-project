...
=========================================
Scrapes Top 250 + General Popular films using Edge + Selenium.

Output:
  nandana_films.csv  -- film details table
  nandana_lists.csv  -- list membership table
...
LISTS_TO_SCRAPE = {
    # Letterboxd Official Top 250 + Top 500 (verified working URLs)
    "Top 250":              "https://letterboxd.com/dave/list/official-top-250-narrative-feature-films/detail/",
    "Top 500":              "https://letterboxd.com/official/list/letterboxds-top-500-films/",
    "Top 250 Most Fans":    "https://letterboxd.com/official/list/top-250-films-with-the-most-fans/",

    # General Popular -- proven working browse format
    "Popular All Time":     "https://letterboxd.com/films/popular/",
    "Popular This Year":    "https://letterboxd.com/films/popular/this/year/",
}

PERSONALITY_LABEL = "Hype Chaser"
DELAY_MIN = 1.0
DELAY_MAX = 2.0
...
def get_film_urls_from_list(driver, list_url: str) -> list[tuple[str, str]]:
    ...
    while True:
        ...
        print(f"    [{list_url.rstrip('/').split('/')[-1]}] Page {page}: {len(items)} items, {len(films)} total")
        page += 1
...
def scrape_all():
    ...
    try:
        ...
        lists_df.to_csv("nandana_lists.csv", index=False)
        print(f"nandana_lists.csv saved -- {len(lists_df)} rows")
        ...
        films_df.to_csv("nandana_films.csv", index=False)
        print(f"\nnandana_films.csv saved -- {len(films_df)} rows")
        print("\nDone. nandana_films.csv and nandana_lists.csv are ready.\n")
