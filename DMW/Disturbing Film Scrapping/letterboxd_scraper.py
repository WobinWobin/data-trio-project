"""
CineMatch - Letterboxd Scraper (Ethan)
=========================================
Scrapes Disturbing + Horror lists using Chrome + Selenium.
Runs 4 browsers in parallel for speed.

Output:
  disturbing_films.csv  -- film details table
  disturbing_lists.csv  -- list membership table
...
LISTS_TO_SCRAPE = {
    # Broad horror/disturbing browse pages
    "Popular Horror":          "https://letterboxd.com/films/popular/genre/horror/",
    "Popular Thriller":        "https://letterboxd.com/films/popular/genre/thriller/",

    # Curated disturbing / extreme lists
    "Most Disturbing":         "https://letterboxd.com/s0m3th1ng/list/disturbing-films-that-you-cant-look-away-from/",
    "Horror Essentials":       "https://letterboxd.com/dave/list/official-top-250-horror-films/",
    "Psychological Horror":    "https://letterboxd.com/el_profeta/list/psychological-horror/",
    "Extreme Cinema":          "https://letterboxd.com/milesraymond/list/extreme-cinema/",
}

PERSONALITY_LABEL = "Disturbing"
DELAY_MIN = 1.0
DELAY_MAX = 2.0
...
def scrape_film_details(driver, film_url: str) -> dict:
    ...
    # Title
    for sel in ["h1.headline-1", "h1.filmtitle span", "h1.filmtitle", "h1[itemprop='name']"]:
    ...
    # avg_rating + num_ratings -- JSON-LD / inline script fallback
    ...
    # num_ratings -- visible text fallback e.g. "1.2M ratings"
    if result["num_ratings"] is None:
    ...
    # num_reviews -- JSON-LD aggregateRating.reviewCount
    try:
...
def scrape_all():
    ...
    lists_df.to_csv("disturbing_lists.csv", index=False)
    print(f"disturbing_lists.csv saved -- {len(lists_df)} rows")
    ...
    films_df.to_csv("disturbing_films.csv", index=False)
    print(f"\ndisturbing_films.csv saved -- {len(films_df)} rows")
    print("\nDone. disturbing_films.csv and disturbing_lists.csv are ready to send.\n")
